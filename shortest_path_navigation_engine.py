    ...
    def _calculate_convergence_proof(self) -> Dict:
        ...
        for i in range(1, len(self.complexity_history)):
            ...
            if D_k1 <= D_k:
                ...
                violations.append(
                    {"iteration": i, "D_k": D_k, "D_k1": D_k1, "violation": D_k1 - D_k}
                )
        ...
        return {
            ...
            "complexity_reduction_percent": (
                (complexity_reduction / self.complexity_history[0] * 100)
                if self.complexity_history[0] > 0
                else 0.0
            ),
...
def demo_vscode_optimization():
    ...
    print(
        f"   • Total Reduction: {proof_d['complexity_reduction']} ({proof_d['complexity_reduction_percent']:.1f}%)"
    )
